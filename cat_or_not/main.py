...
from app_tools import save_folder_exists, model_exists
...
def result():
    if request.method == "POST":
        ...
        file.save(file_path)
        database.add_result(file.filename)
...
def results(id):
    filename = os.path.join("../static/photos/user_photos/", database.get_result(id))
