...
class ImageValidator():
    ...
    def is_image(self, file, filename):
        size = file_size(file)
        if size > 1024 * 1024 * 4:
            self.raport.append('File must have size <= 4MB')
        if self.has_no_empty_filename(file) and self.has_image_extension(filename):
            return True
        elif self.has_no_empty_filename(file) and (not self.has_image_extension(filename)):
    ...
    def has_image_extension(self, filename):
